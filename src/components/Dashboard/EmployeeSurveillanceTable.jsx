...
import { Search, Download, FileText, FileSpreadsheet, Printer, ChevronDown } from "lucide-react";
...
export default function EmployeeSurveillanceTable({ rows = [] }) {
  ...
  function exportCSV() {
    const blob = new Blob([buildCSV(filteredRows)], { type: "text/csv" });
    download(blob, "employee_surveillance.csv");
    setShowExport(false);
  }
  ...
  function exportExcel() {
    ...
    download(blob, "employee_surveillance.xlsx");
    setShowExport(false);
  }
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
            ...
            <div className="relative flex-1 sm:flex-initial">
              ...
              <input
                ...
                placeholder="Search employee, department, exposure..."
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gradient-to-r from-[#003E77] to-[#0056A8] text-white">
              <th className="px-6 py-4 text-left font-semibold tracking-wide">Employee</th>
              <th className="px-6 py-4 text-left font-semibold tracking-wide">Exposure</th>
              <th className="px-6 py-4 text-left font-semibold tracking-wide">Risk Level</th>
              <th className="px-6 py-4 text-left font-semibold tracking-wide">Tests Required</th>
              <th className="px-6 py-4 text-left font-semibold tracking-wide">Last Blood Test</th>
              <th className="px-6 py-4 text-left font-semibold tracking-wide">Next Due</th>
            </tr>
          </thead>
                ...
                <tr key={idx} className="hover:bg-blue-50/30 transition-colors duration-150">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-gray-900">
                      {cell(r.employee, "Employee name")}
                    </div>
                    <div className="text-xs text-blue-600 mt-1 font-medium">
                      {cell(r.department, "Department")}
                    </div>
                  </td>

                  <td className="px-6 py-4 text-gray-700">
                    {cell(r.exposure, "Exposure details")}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {r.risk ? (
                      ...
                      <span className={placeholderCell}>
                        Risk level
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 text-gray-700">
                    {cell(r.tests, "Required tests")}
                  </td>

                  <td className="px-6 py-4 text-gray-700">
                    {cell(r.lastTest, "Last test date")}
                  </td>

                  <td className="px-6 py-4 font-semibold text-gray-900">
                    {cell(r.nextDue, "Next due date")}
                  </td>
                </tr>
              ))
            ) : (
                ...
                <td colSpan={6} className="px-6 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-gray-400 italic text-base">
                      {search.trim() ? "No matching employees found." : "Employee surveillance data will appear here."}
                    </p>
                  </div>
                </td>
              </tr>
